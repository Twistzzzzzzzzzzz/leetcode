# Two Pointers

这个专题练习双指针，包括原地覆盖、左右指针、快慢指针等写法。

## 题目顺序

| 顺序 | 题号 | 题目 | 代码 | 笔记 |
| --- | --- | --- | --- | --- |
| 1 | 27 | Remove Element | `p0027_remove_element.py` | `p0027_remove_element.md` |
| 2 | 75 | Sort Colors | `p0075_sort_colors.py` | `p0075_sort_colors.md` |
| 3 | 344 | Reverse String | `p0344_reverse_string.py` | `p0344_reverse_string.md` |
| 4 | 125 | Valid Palindrome | `p0125_valid_palindrome.py` | `p0125_valid_palindrome.md` |
| 5 | 680 | Valid Palindrome II | `p0680_valid_palindrome_ii.py` | `p0680_valid_palindrome_ii.md` |
| 6 | 1768 | Merge Strings Alternately | `p1768_merge_strings_alternately.py` | `p1768_merge_strings_alternately.md` |
| 7 | 88 | Merge Sorted Array | `p0088_merge_sorted_array.py` | `p0088_merge_sorted_array.md` |
| 8 | 26 | Remove Duplicates from Sorted Array | `p0026_remove_duplicates_from_sorted_array.py` | `p0026_remove_duplicates_from_sorted_array.md` |
| 9 | 167 | Two Sum II - Input Array Is Sorted | `p0167_two_sum_ii_input_array_is_sorted.py` | `p0167_two_sum_ii_input_array_is_sorted.md` |
